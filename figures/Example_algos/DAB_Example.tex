\begin{tikzpicture}[ thick, main node/.style={circle, draw, minimum size=1.2cm}]
    ...
    \draw[->] (C1) to[out=120, in=240] node[left] {$t_{A,B}$} (A1);
    \draw[dashed, ->] (C1) to[out=60, in=300] node[right] {$\frac{l_{C}-l_{A}}{2}$} (A1);
    \draw[dashed, ->] (B1) to[out=210, in=330] node[above] {$\frac{l_{B}-l_{A}}{2}$} (A1);
    \draw[dashed, ->] (D1) to[out=120, in=330] node[right] {$\frac{l_{D}-l_{A}}{2}$} (A1);




    
    \draw[->] (C2) to[out=120, in=240] node[left] {$29.5$} (A2);
    \draw[dashed, ->] (C2) to[out=60, in=300] node[right] {$29.5$} (A2);
     \draw[dashed, ->] (B2) to[out=210, in=330] node[above] {$6$} (A2);
     \draw[dashed, ->] (D2) to[out=120, in=330] node[right] {$20.5$} (A2);

    \node at (2.5,6) {Initial Setup};
    \node at (9.5,6) {Setup after round 1};

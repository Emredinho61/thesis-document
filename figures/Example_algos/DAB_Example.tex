\begin{tikzpicture}[ thick, main node/.style={circle, draw, minimum size=1.2cm}]
    ...
    \draw[->] (C1) to[out=120, in=240] node[left, scale=0.8] {$29.5$} (A1);
    \draw[dashed, ->] (C1) to[out=60, in=300] node[right, scale=0.8] {$29.5$} (A1);
    \draw[dashed, ->] (B1) to[out=210, in=330] node[above, scale=0.8] {$6$} (A1);
    \draw[dashed, ->] (D1) to[out=120, in=330] node[right, scale=0.8] {$20.5$} (A1);
    ...
    \begin{scope}[shift={(3,-2)}]
        ...
        % Load notation
        \node at (4,0) {$l_{i}$ : load of node i};
        \node at (5,-1) {$t_{i,j}$ : transfered load from i to j};
    \end{scope}
